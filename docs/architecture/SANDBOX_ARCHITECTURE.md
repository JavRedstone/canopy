...
4. **`DockerSandboxRunner`** ([`services/sandbox_runner/sandbox_runner/runner.py`](../../services/sandbox_runner/sandbox_runner/runner.py)) does the actual execution:
   - Creates a **fresh container** per run from a curated `SandboxEnvironment` registry (`python-basic`, `javascript-basic`, `go-basic`, `cpp-basic` — each its own Dockerfile under [`sandbox_image/`](../../services/sandbox_runner/sandbox_image/), e.g. `python-basic` is `python:3.13-slim` + `pytest`/`pytest-timeout`), building the requested one lazily on first use if it doesn't exist yet. There is no path to an unregistered/arbitrary image.
   - Copies the submitted files in via Docker's `put_archive` (a tar stream built in-memory — nothing touches the host filesystem).
   - Runs the environment's own test/script command (e.g. `pytest -v -p no:cacheprovider .` / `python -u <entry_path>` for `python-basic`; `node --test` / `node <entry_path>` for `javascript-basic`; `go test ./...` / `go run <entry_path>` for `go-basic`; `g++ ... *.cpp -o ... && ...` compile-then-run, linked against a bootstrapped doctest `main()`, for `cpp-basic`) as the container's command.
