...
class LessonBundle(BaseModel):
    ...
    public_test_files: list[WorkspaceFile] = Field(min_length=1, max_length=3)
    ...
    def paths_are_consistent(self) -> "LessonBundle":
        ...
        public_test_paths = {file.path for file in self.public_test_files}
        ...
        if len(public_test_paths) != len(self.public_test_files):
            raise ValueError("Public test file paths must be unique.")
        ...
        if starter_paths & public_test_paths:
            raise ValueError("Public test files must not collide with starter file paths.")
        if starter_paths & test_paths:
            raise ValueError("Test files must not collide with starter file paths.")
        if public_test_paths & test_paths:
            raise ValueError("Public and hidden test files must not share a path.")
        if not all(path.startswith("test_") or path.endswith("_test.py") for path in public_test_paths):
            raise ValueError("Every public test file must be pytest-discoverable (test_*.py) since learners run them individually.")
...
def reference_workspace(bundle: LessonBundle) -> list[WorkspaceFile]:
    """The reference solution plus every test (public and hidden) it must pass — what the sandbox validates."""
    return [*bundle.reference_solution_files, *bundle.public_test_files, *bundle.test_files]
